...
$('label').on('click',function(e){
  ...
  if($(this).children('.checkbox-img').hasClass('checked')) {
    // .checkbox-img가 ckecked 클래스를 가지고 있을 때 실핼할 코드
    $(this).children('input[type="checkbox"]').attr('checked', 'checked');
  } else {
    // .checkbox-img가 ckecked 클래스를 가지고 있지 않을 때 실핼할 코드
...
// #submut 버튼을 클릭 했을 때 필수동의 사항에 모두 체크했는지 확인
// 모두 체크되어 있으면 통과
// 체크 안된 게 있다면 #red-alert을 보여주고 전송을 중지
$('#submut').on('click', function(e) {
  let req = $('.req').length;
